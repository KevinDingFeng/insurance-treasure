 ...
 * All rights reserved.
 *
 * $Id: StringUtil.java 3120 2007-09-06 06:54:20Z ganja $
 */
 /* 文档密级：秘密 */
...
public class StringUtil {
	...
	public static void main(String args[]) throws Exception {
		// log.info("11==="+base64DeCode("QVNISEoxMDk5ODA3RDEwMjIzNEo="+"------"+"MzAxMDEwMA=="+"------"+"QlNISEoxMDk5ODA3RDEwNTUwMEo="));
		// log.info("22==="+base64DeCode("???????????????,????????!"));//??????????
		// log.info("55==="+padRight("sru6z7jx",4));
		...
		// log.info(ss.substring(10,11));
		log.info("解密 >>>>>>>>");
		log.info(base64DeCode("QVNISDI2MzA0MTA5RDgyODUwMU0="));
		log.info(base64DeCode("Tg=="));
		log.info(base64DeCode("57O757uf5Lit5LiN5a2Y5Zyo6K+l5L+d5Y2V5om55Y2V6K6w5b2VLOaXoOazlei/m+ihjOafpeivouaTjeS9nCE="));
		log.info(base64DeCode("Mw=="));

		log.info("加密 >>>>>>>>");
		log.info(base64EnCode("VIP00000000000000011"));
		log.info("========");
		log.info(base64EnCode("3040100"));
		log.info(base64EnCode("ANIB92024209D813558T"));
		log.info(base64EnCode("Mw=="));
		// log.info(getStringLength("23423fsdf右粗顺轩困在;"));
	}
...
//				year = java.lang.Integer.parseInt(date.substring(0, 4));
	...
	public static String testPrint(String str, int len) {
		// String returnString="Covering All Risks and War Risks as per Ocean
		// Marine Cargo Clauses and Ocean Marine Cargo War Risks
		// Clauses(1／1／1981) of China Pacific Property Insurance Co.,Ltd.";
		str = str.replaceAll("\r\n", "");
		String temp = "";
		int len1 = str.length();
		int j = 0;
		...
		if (j >= len1) {
			...
			if (!"".equals(String.valueOf(str.charAt(j)).trim())) {
				for (j = len - 1; j > 0; j--) {
					if ("".equals(String.valueOf(str.charAt(j - 1)).trim())) {
		...
		if (j1 >= len1) {
			...
			if (!"".equals(String.valueOf(str.charAt(j1)).trim())) {
				for (; j1 > j; j1--) {
					if ("".equals(String.valueOf(str.charAt(j1 - 1)).trim())) {
						temp += str.substring(j, j1) + "\r\n";
		...
		j2 = j1 + len - 1;
		if (j2 >= len1) {
			temp += str.substring(j1) + "\r\n";
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j2)).trim())) {
				for (; j2 > j1; j2--) {
					if ("".equals(String.valueOf(str.charAt(j2 - 1)).trim())) {
						temp += str.substring(j1, j2) + "\r\n";
						break;
					}
				}
			} else {
		...
		j3 = j2 + len - 1;
		if (j3 >= len1) {
			temp += (str.substring(j2) + "\r\n").trim();
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j3)).trim())) {
				for (; j3 > j2; j3--) {
					if ("".equals(String.valueOf(str.charAt(j3 - 1)).trim())) {
						temp += str.substring(j2, j3) + "\r\n";
						break;
					}
				}
			} else {
		...
		j4 = j3 + len - 1;
		if (j4 >= len1) {
			temp += (str.substring(j3) + "\r\n").trim();
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j4)).trim())) {
				for (; j4 > j3; j4--) {
					if ("".equals(String.valueOf(str.charAt(j4 - 1)).trim())) {
						temp += str.substring(j3, j4) + "\r\n";
						temp = temp.trim();
						break;
					}
				}
		...
		j5 = j4 + len - 1;
		if (j5 >= len1) {
			temp += (str.substring(j4) + "\r\n").trim();
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j5)).trim())) {
				for (; j5 > j4; j5--) {
					if ("".equals(String.valueOf(str.charAt(j5 - 1)).trim())) {
						temp += str.substring(j4, j5) + "\r\n";
						break;
					}
				}
			} else {
		...
		j6 = j5 + len - 1;
		if (j6 >= len1) {
			temp += (str.substring(j5) + "\r\n").trim();
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j6)).trim())) {
				for (; j6 > j5; j6--) {
					if ("".equals(String.valueOf(str.charAt(j6 - 1)).trim())) {
						temp += str.substring(j5, j6) + "\r\n";
		...
		j7 = j6 + len - 1;
		if (j7 >= len1) {
			temp += (str.substring(j6) + "\r\n").trim();
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j7)).trim())) {
				for (; j7 > j6; j7--) {
					if ("".equals(String.valueOf(str.charAt(j7 - 1)).trim())) {
						temp += str.substring(j6, j7) + "\r\n";
						break;
					}
				}
			} else {
		...
		j8 = j7 + len - 1;
		if (j8 >= len1) {
			temp += str.substring(j7) + "\r\n";
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j8)).trim())) {
				for (; j8 > j7; j8--) {
					if ("".equals(String.valueOf(str.charAt(j8 - 1)).trim())) {
						temp += str.substring(j7, j8) + "\r\n";
						break;
					}
				}
			} else {
		...
		j9 = j8 + len - 1;
		if (j9 >= len1) {
			temp += (str.substring(j8) + "\r\n").trim();
			return temp;
		} else {
			if (!"".equals(String.valueOf(str.charAt(j9)).trim())) {
				for (; j9 > j8; j9--) {
					if ("".equals(String.valueOf(str.charAt(j9 - 1)).trim())) {
						temp += str.substring(j8, j9) + "\r\n";
						break;
					}
				}
			} else {
	...
	/**
	 * 得到一个这符串的长度，一个汉字按4个字节算
	 * 
	 * @param arg
	 * @return
	 */
	public static int getStringLength(String arg) {
		int intLength = 0;
		for (int i = 0; i < arg.length(); i++) {
			if ((arg.charAt(i) < 0) || (arg.charAt(i) > 255)) {
	...
	public static String[] split(String pStrSrc, String pStrReg) {
		...
		if (StringUtils.isNotBlank(pStrSrc) && StringUtils.isNotBlank(pStrReg)) {
	...
	public static boolean isEmpty(String s) {
		return s == null || s.trim() == null || s.trim().length() == 0;
	}
}
